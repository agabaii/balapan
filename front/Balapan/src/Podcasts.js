...
import { Headphones, Play, Clock, Star, BookOpen, Home } from 'lucide-react';
      ...
      <div className="flex" style={{ backgroundColor: '#FFFECF' }}>
        ...
        <div className="flex-1 px-6 py-6" style={{ backgroundColor: '#FFFECF' }}>
          <div className="rounded-2xl p-6 mb-8 bg-gradient-to-r from-pink-50 to-purple-50 shadow-sm border border-white">
            <div className="text-center">
              ...
              <p className="text-base font-medium text-gray-500">
                Слушайте и учите язык с удовольствием
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-12">
            {podcastsData
              .filter(podcast => {
                const selectedLanguage = localStorage.getItem('selectedLanguage') || 'kazakh';
                return podcast.language === selectedLanguage;
              })
              .map((podcast) => (
                <Link
                  key={podcast.id}
